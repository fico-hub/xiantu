...
**体力（HP）：**
- 进入城市秘境探索时，有 20% 概率遭遇危险扣减体力（-5~15）
- 长途移动到达时有概率损耗体力（长途更高概率，最高 40%）
- 体力为 0 时无法进入秘境（会被拒绝）
- 在洞府打坐或离线修炼（`POST /api/cultivate/offline`）时自动恢复
- 恢复速率：每游戏年恢复最大体力的 **5%**，最低 1 点

**灵力（Mana）：**
- 在城市秘境结算时，有 **30% 概率** 触发灵力加成：消耗 10% 最大灵力，获得额外天材地宝
- 灵力不足时直接跳过（不报错，只是拿不到额外材料）
- 同样通过洞府打坐或离线修炼恢复（5%/年）
- 突破境界时 max_hp / max_mana 自动更新至新境界上限

`GET /api/profile` 返回中包含：`hp`, `maxHp`, `mana`, `maxMana`, `hpPercent`, `manaPercent`
...
POST /api/city-realms/:id/enter     # 进入（消耗神识值，有概率扣体力）
POST /api/city-realms/:id/exit      # 结算收益+获得叙事事件种子（有概率消耗灵力获加成）
